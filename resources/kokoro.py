...
def rvc_queue(q, done_q, rvc_model_path):
    while True:  # Could create zombie processes
        ...
        #vc_convert(model_path=rvc_model_path,input_path=audio_path, output_dir_path=opt_dir)
...
class Kokoro:
    def __init__(self, 
        ...
        # Read in keys
        if self.gptmodel == "gpt-3.5-turbo":
        ...
        try:
            ...
            elif os.environ['TTS']== "tortoise" or "rvc":
 ...
 # Methods the assistants rely on------------------------------------------------------------------------------------------------------------------
    ...
    def response_completion(self, append=True):
        ...
        elif self.gptmodel == "textgenerator":
            self.model = WhisperModel(self.model_size, device=self.model_device, compute_type=self.model_compute_type)
            print(Fore.WHITE)
            response = asyncio.run(self.print_response_stream(self, self.messages))       
        if append:
    ...
    def speech_recognition(self, audio):
        ...
        try:
            if self.speech_recog == "whisper":
                ...
                return user_input
            elif self.speech_recog =="fasterwhisper":
                from faster_whisper import WhisperModel
                if audio:
                    user_input, language, language_probability = self.fasterwhisper(self, audio, self.model,)
                    ...
                    if "en" in language or "pt" in language and language_probability >= 0.4:
                        return user_input, language, language_probability
                    else:
    ...
    def fasterwhisper(self, audio, model):
        response = ""
        with open('speech.wav','wb') as f:
            f.write(audio.get_wav_data())
        speech = open('speech.wav', 'rb')
    ...
    async def print_response_stream(self, user_input):
        cur_len = 0
        final_message = ''
        #SEND TO Text-Generator-WebUI-API IP
        async for new_history in connect.run(user_input):
