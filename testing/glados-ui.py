from modules.glados_ui.text_resources import aperture, help_text, login_text, recipe
import sys
import random
...
from textual.widgets import Digits, Footer, Header, Label, RichLog, Static, Input
from modules.glados_ui.text_resources import aperture, help_text, login_text, recipe
# Custom Widgets
...
class SplashScreen(Screen):
    ...
    with open(Path("./modules/glados_ui/images/splash.ansi"), 'r', encoding='utf-8') as f:
...
class GladosUI(App):
    ...
    CSS_PATH = "modules/glados_ui/glados.tcss"
    ...
    with open(Path("./modules/glados_ui/images/logo.ansi"), 'r', encoding='utf-8') as f:
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="body"):  # noqa: SIM117
            with Horizontal():
                ...
                yield Input(id="input", placeholder="Type your command here...")
    ...
    def on_load(self) -> None:
        """Called when the app has been started but before the terminal is in app mode."""
        logger.remove()
        fmt = (
    ...
    def start_glados(self):
        """Start the GLaDOS worker process."""
        #self.glados = self.run_worker(GLaDOS.run, exclusive=True, thread=True)

    def on_input_submitted(self, message: Input.Submitted) -> None:
        """Handle the submitted input."""
        command = message.value.strip()
        if command:
            self.query_one(Printer).write(f"User typed: {command}")
            # Handle the command here, e.g., start GLaDOS, quit, or any other action
